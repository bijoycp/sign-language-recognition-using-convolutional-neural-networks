...
out_label=['T','E',' ','R','Q','D','-','G','I','O','O','C','W','Y','V','N','H','P','S','A','L','K','X','M','_','F','U']
...
while(True):
    ...
    if num_frames < 30:
        ...
        if hand is not None:
            ...
            print(str(np.argmax(model_out))+" "+str(out_label[pnb-1]))

            pre.append(out_label[pnb-1]) 


            cv2.putText(clone,
                   '%s ' % (str(out_label[pnb-1])),
    ...
    if flag==1:
        ...
        cv2.putText(clone,
                   ' : %s ' % (str(c[0])),
                   (500, 150), cv2.FONT_HERSHEY_PLAIN,5,(0, 255, 0))
        flag1=0
